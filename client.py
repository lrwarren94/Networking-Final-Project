...
def sr_protocol(server_address, server_port, client_rcv_port, client_send_port, file_name, window_size):
    sender = SRSender(server_address, server_port, client_send_port, window_size)
    buffer = Queue()
    buffer.daemon = True
    data = ''
    receiver = SRReceiver(buffer, client_rcv_port, window_size)
    t1 = begin_receive_thread(receiver)
    t1.start()
    sender.send(str(file_name + '@@@' + str(client_rcv_port)))
    sender.send('SEND COMPLETE')
...
def main():
    ...
    if protocol == 1:
        ...
        sr_protocol(server_address, server_port, client_rcv_port, client_send_port, file_name, 10)    
