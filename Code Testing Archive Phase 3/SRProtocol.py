...
class SRReceiver:
    ...
    def receive(self):
        ...
        while self.is_receiving():
            ...
            if not self._is_corrupt(recv, checksum):
                        ...
                        if rcv_seq_num == self._send_base:
                            ...
                            while consecutive:
                                if self._seq_buffer[self._send_base] != 'INIT':
                                    self._delivery_buffer.put(self._seq_buffer[self._send_base])
...
class SRSender:
    ...
    def send(self, data):
        ...
        for i in range(0, self._seq_buffer_size): #set up the sequence buffer for the first time
            self._seq_buffer[i][0] = data_to_send[i]
        dts_index += self._seq_buffer_size
                            ...
                            while consecutive:
                                if self._seq_buffer[self._send_base][1] == 1:
                                    ...
                                    if self._send_base == self._seq_buffer_size - 1:
                                        ...
                                        self._send_base += 1
                                    if sent_packets < packets_to_send:
